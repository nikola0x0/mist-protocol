#!/bin/bash
# ec2-rebuild.sh - Rebuild XWallet enclave on EC2 from local code
...
if ! ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=10 "ec2-user@$ELASTIC_IP" "echo 'SSH OK'" 2>/dev/null; then
    echo "ERROR: Cannot connect to EC2 instance at $ELASTIC_IP"
    echo "Make sure the instance is running. Try: ./ec2-start.sh"
    exit 1
fi
...
echo "To:   ec2-user@$ELASTIC_IP:$REMOTE_DIR"
echo ""
...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "ec2-user@$ELASTIC_IP" << 'ENDSSH'
set -e
cd ~/nautilus-xwallet
...
ENCLAVE_OUTPUT=$(ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "ec2-user@$ELASTIC_IP" \
...
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SECRETS_FILE" "ec2-user@$ELASTIC_IP:/tmp/secrets.json"
rm "$SECRETS_FILE"
