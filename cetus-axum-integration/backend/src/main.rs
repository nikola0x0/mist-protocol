...
async fn build_swap_transaction(
    ...
    info!(
        "Building swap transaction for user {} - {} {} to {} (a_to_b: {})",
        request.user_address, request.amount, request.token_a, request.token_b, request.a_to_b
    );

    // Find the pool
    let pool = match CetusService::find_pool(&state.http_client, &request.token_a, &request.token_b)
        .await
    {
        Ok(Some(p)) => p,
        Ok(None) => {
    ...
    // Calculate expected output (simplified - in production use actual calculations)
    let expected_output = (request.amount as f64 * 0.997) as u64; // Example with 0.3% fee
    let min_output = (expected_output as f64 * (1.0 - request.slippage)) as u64;

    // Build unsigned transaction using flash swap (direct CLMM approach)
    match transaction::build_flash_swap_transaction(
        &state.sui_client,
        &state.config,
        ...
        Ok(tx_data) => {
            ...
            Ok(Json(BuildSwapResponse {
                ...
                pool_info: PoolInfo {
                    ...
                    expected_output,
                    price_impact: 0.1, // Calculate properly in production
                },
